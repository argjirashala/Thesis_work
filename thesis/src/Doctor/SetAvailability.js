...
const handleSaveAvailability = async () => {
  ...
  <h2>Set Your Availability</h2>
              ...
              <div className="availability-section">
                  ...
                  <DatePicker  className='input input-field' selected={selectedDate} onChange={setSelectedDate} />
                  <br></br>
    <div className="time-inputs">
        <div>
            <label>Start Time:</label>
            <input type="time" className='input input-field' required value={startTime} onChange={e => setStartTime(e.target.value)}/>
        </div>
        <div>
            <label>End Time:</label>
            <input type="time" className='input input-field' required value={endTime} onChange={e => setEndTime(e.target.value)} />
        </div>
    </div>
    <div className="add-slot-button-container">
        ...
        {selectedTimeSlots.map((slot, index) => (
            <div key={index} className="slot-item">
                ...
                <button className="remove-slot-button" onClick={() => handleRemoveSlot(slot)}>Remove</button>
            </div>
        ))}
    </div>
